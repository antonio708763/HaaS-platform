...
- Proxmox host is connected to LAN via a single physical NIC (`nic0`)
...
- `tap900i0` is present (VMID 900 connected to bridge)
...
Note:
Docker-created networks exist in the guest (e.g., `docker0` 172.17.0.0/16 and additional `br-*` networks). These are internal to the VM and do not affect LAN addressing.
...
- VM uses DHCP; future “reference build” may standardize on static IPs or DHCP reservations.
- DNS is currently router-provided; future reference build may introduce dedicated DNS (e.g., AdGuard Home / Pi-hole / Unbound) with documented failover behavior.
